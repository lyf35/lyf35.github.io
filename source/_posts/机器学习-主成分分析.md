...
我们将原始数据记作$\boldsymbol{x}_1,\boldsymbol{x}_2,\dots,\boldsymbol{x}_N$。假设现在构造了一个用来近似表示$\boldsymbol{x}_i$的模型：$f(\boldsymbol{\lambda})=\mu + \boldsymbol{V}_{q}\boldsymbol{\lambda}$，其中$\mu$是一个$p$维空间中的向量，$\boldsymbol{V}_q$是一个$p\times q$的矩阵，其中每一列为正交的单位向量，函数的输入参数$\boldsymbol{\lambda}$即为压缩后的$q$维向量。
