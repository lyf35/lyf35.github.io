...
实质上，在指令执行层，硬件的调度是基于半个线程束，而不是整个线程束。因此，如果能将半个线程束中的连续线程划分到同一个分支，这样硬件就可以同时执行分支结构中两个不同条件的分支块。就例如下面的代码段：
...
- `Ns`是一个`size_t`类型的变量，声明动态分配的共享存储器大小，默认为0
- `s`的类型为`cudaStream_t`，用来声明执行的流，默认为0
...
12. [“CUDA Tutorial” (jhui.github.io)](https://jhui.github.io/2017/03/06/CUDA/)
13. [CUDA学习笔记(5) 原子操作_GongBingjian-CSDN博客_cuda 原子操作](https://blog.csdn.net/q583956932/article/details/78826987)
14. [cublas 实战_thesby的专栏-CSDN博客_cublas](https://blog.csdn.net/thesby/article/details/52728529)
15. The CUDA Handbook
