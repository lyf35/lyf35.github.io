...
**支持向量机**（Support Vector Machine，SVM）是一种二分类模型，它的学习思想是在空间内找到一个超平面$\boldsymbol{x}\boldsymbol{w}^T+b=0$（其中$\boldsymbol{x}=(x_1,x_2,\dots,x_n)$代表模型的输入向量，$\boldsymbol{w}=(w_1,w_2,\dots,w_n)$与$b$为模型参数），能够使得两类样本与超平面的间距最大化。落在超平面两侧的样本，分别对应于$y=+1$和$y=-1$两种不同的分类。也就是说，在学得超平面的表达式之后，支持向量机模型的分类函数可以写为：
$$
y=
\begin{cases}
