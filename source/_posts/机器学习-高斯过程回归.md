...
$$
而我们从$\hat{\boldsymbol{\mu}}$的表达式中也可以看出，这一表达式的形式与加了核函数的岭回归相同。也就是从高斯过程回归的角度考虑的话，核岭回归其实一定程度上等价于高斯过程回归。二者的区别在于，在高斯过程回归中，核函数的参数可以通过模型的学习过程中自动优化，而核岭回归中核函数的参数需要自己设定。
...
axes.fill(np.concatenate([x_pred,x_pred[::-1]]),np.concatenate([y_pred-1.9600*sigma,(y_pred+1.9600*sigma)[::-1]]),alpha=0.2,fc='b',ec='None',label='95% Confidence Interval')
